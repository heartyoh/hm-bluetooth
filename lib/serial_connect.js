...
var COMMAND_TYPE_COMMAND = 1;
var COMMAND_TYPE_GET = 2;
var COMMAND_TYPE_SET = 3;
var COMMAND_TYPE_WAKE = 4;
...
SerialConnect.prototype.open = function(cb) {
  ...
  this.connect.on('data', function(data) {
    if(self.patternType != COMMAND_TYPE_AT && self.patternType != COMMAND_TYPE_WAKE
      && !self.currentPattern) {
...
SerialConnect.prototype.get = function(cmd) {
  ...
  if(!pattern && cmd.endsWith('?')) {
    cmd = cmd.substr(0, cmd.length - 1);
    pattern = CommandPattern[cmd];
  }
...
SerialConnect.prototype.command = function(cmd) {
  ...
  if(command.params && command.params.length > arguments.length - 1) {
  ...
  this.currentPattern = pattern;
  this.patternType = COMMAND_TYPE_COMMAND;
...
SerialConnect.prototype.wake = function() {

  this.patternType = COMMAND_TYPE_WAKE;

  var self = this;

  /* 아무 문자나 80자를 보냄 */
  var cmd_str = '1234567890123456789012345678901234567890';
  cmd_str += '1234567890123456789012345678901234567890';
...
function complete(connect) {
  ...
  case COMMAND_TYPE_COMMAND:
    ...
    prefix = connect.currentPattern.command.result.prefix;
  ...
  if(prefix && result.indexOf(prefix) != 0) {
    ...
    connect.fire('error', "Invalid result (OK required.) : " + result + " (" + prefix + ")");
  } else {
